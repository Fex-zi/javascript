...
const interestedIN = prompt(
  "what do you wanna know about ify, choose firstname, lastname, age, job, and friends"
);

const check = ify[interestedIN]
  ? console.log(ify[interestedIN])
  : `Does not exist`;
...
console.log(
  `${ify["firstname"]} has ${ifyBest} friends, and his best friend is called ${ifyFriend}`
);
...
const DriversL = ify["hasDrivesLicense"]
...
for (let i = 0; i < ify.length; i++) {
  console.log(ify[i]);

  // filling array and add more
  types[i] = ify[i];
  types.push(`hello`);
}
...
for (let i = 0; i < years.length; i++) {
  //ages.push (2025 - years[i]);//same thing like below
  ages[i] = 2025 - years[i]; //same thing
}
...
for (let i = 0; i < ify.length; i++) {
  if (typeof ify[i] !== "string") continue; // don't show number

  console.log(ify[i], typeof ify[i]);
}

console.log("----BREAK WITH NUMBER----");
for (let i = 0; i < ify.length; i++) {
  if (typeof ify[i] === "number") break; // stop when number found
...
//backward count || start || stop || loop
for (let i = ify.length - 1; i >= 0; i--) {
  console.log(i, ify[i], typeof ify[i]);
}
...
for (let exercise = 1; exercise <= 3; exercise++) {
  ...
  for (let rep = 1; rep < 6; rep++) {
    console.log(`Exercise ${exercise} Lifting weigt repitition ${rep}🏋️`);
  }
}
...
while (dice !== 6) {
  ...
  if (dice === 6) console.log(`----Loop ended because Dice was ${dice}.....`);
}
...
for (let i = 0; i < bills.length; i++) {
  ...
  sum += totals[i];
  console.log(`Tips for ${bills[i]} is ${tips[i]}, and total is ${totals[i]}`);
}
console.log(`---Total profits for today is ${sum}---`);
